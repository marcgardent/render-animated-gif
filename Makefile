...
CURRENT_VER  := $(shell grep '^version\s*=' $(SRC_DIR)/blender_manifest.toml | cut -d'"' -f2)
VERSION      ?= $(CURRENT_VER)
...
.PHONY: all help build build-light install install-dev link unlink uninstall wheels validate test version clean

all: help

## help: Display this help message
help:
	@echo "$(BOLD)Extension : $(CYAN)$(EXT_ID)$(RESET) $(YELLOW)v$(CURRENT_VER)$(RESET) (Blender $(BLENDER_VER))"
	@echo ""
	...
	@echo "  $(GREEN)make version VERSION=X.Y.Z$(RESET)  - Set project version, commit and create git tag"
...
## version: Set project version, commit and create git tag (usage: make version VERSION=2.0.0)
version:
	@if [ "$(origin VERSION)" != "command line" ] || [ -z "$(VERSION)" ]; then \
		echo "$(RED)Error: VERSION is required on the command line.$(RESET)"; \
		echo "Usage: make version VERSION=2.0.0"; \
		exit 1; \
	fi
	@CLEAN_VER=$$(echo "$(VERSION)" | sed 's/^v//'); \
	TAG="v$$CLEAN_VER"; \
	if git rev-parse "$$TAG" >/dev/null 2>&1; then \
		echo "$(RED)Error: Git tag '$$TAG' already exists.$(RESET)"; \
		exit 1; \
	fi; \
	echo "$(CYAN)==> Updating version to $$CLEAN_VER in manifest and __init__.py...$(RESET)"; \
	$(PYTHON) $(SCRIPTS_DIR)/set_version.py "$$CLEAN_VER" || exit 1; \
	echo "$(CYAN)==> Committing version changes...$(RESET)"; \
	git add $(SRC_DIR)/blender_manifest.toml $(SRC_DIR)/__init__.py; \
	git commit -m "chore(release): bump version to $$CLEAN_VER" || true; \
	echo "$(CYAN)==> Creating git tag $$TAG...$(RESET)"; \
	git tag -a "$$TAG" -m "Release $$TAG"; \
	echo "$(GREEN)==> Successfully updated to version $$CLEAN_VER and created tag $$TAG!$(RESET)"; \
	echo "$(YELLOW)To push to GitHub and trigger release workflow, run:$(RESET)"; \
